...
ggplot(data=Q1,aes(x=MaritalStatus,fill=HappyIndex))+geom_bar()+ggtitle("Does Marital Status affect Individual Happiness?")

ggplot(data=Q1,aes(x=MaritalStatus,fill=HappyIndex))+geom_bar(position = "fill")+ggtitle("Does Marital Status affect Individual Happiness?")
```
...
The graph below shows that happiness level accross all kinds of marital status is the same. This could be due to the fact that these relationship decision are usually mutual or we have bad data. 

```{r}
 Q2<-GSS %>%
  select(MaritalStatus, HappyIndex, Sex)

ggplot(data=Q2,aes(x=MaritalStatus,fill=HappyIndex))+geom_bar(position = "fill") + facet_wrap(~Sex)+ggtitle("Do different genders have different happiness levels in regards to their Marital Status?")
```
...
People who’re the most happiest are “not strong democrats” and “not strong republicans”.

But interestingly some of the people who consider themselves as “not strong democrats” aren’t very happy but the  same can’t be said for the republican supporter.

People who’re clueless about their happiness are majority of independent party supporters
...
ggplot(data = counts, aes(x = HappyIndex, y = counts, fill = Pol_Affl)) + geom_bar(stat = "identity", position = position_dodge())+ggtitle("Does Political Party support affect personal happines?")

ggplot(data = GSS, aes(x = HappyIndex, fill = Pol_Affl)) + geom_bar(position = "fill")+ggtitle("Does Political Party support affect personal happines?")
```
